import { Injectable } from '@angular/core';
import { Observable } from 'rxjs';
import { BrowserModule } from '@angular/platform-browser';
import { HttpClient, HttpEventType, HttpResponse, HttpHeaders } from '@angular/common/http';
//import { Commande } from '../app/commande';
import { CommandeRecap } from '../app/models/CommandeRecap';

@Injectable({
  providedIn: 'root'
})
export class CantiniereServiceService {

  api_url = "http://localhost:8080/lunchtime/";
  token = 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9..j-araqREHrebVOj_0QCSNYlj_Bz-chdyPN3vTVUzniW2Fhn16-FF1kk_SpOOgFXgz19Swz-YSAa-H72NLaef_Q';
  
  constructor(public http: HttpClient) { }

  findAll(): Observable<CommandeRecap[]> {
    return this.http.get<CommandeRecap[]>(`http://localhost:8080/lunchtime/order/findall`);
  }

  async getMenu(id_menu : number): Promise<any>{
    return this.http.get<any>(this.api_url+"menu/find/1"+id_menu).toPromise();
  }

  async getMenus(): Promise<any>{
    return this.http.get<any>(this.api_url+"menu/findall",
    {
      headers: new HttpHeaders({
        'Content-Type' : 'application/json',
        'Authorization' : this.token
      })
    }).toPromise();
  }

  async getImageMenu(id_menu : number): Promise<any>{
    return this.http.get<any>(this.api_url+"menu/findimg/"+id_menu).toPromise();
  }

  async getImageMenus(id_menu : number): Promise<any>{
    return this.http.get<any>(this.api_url+"menu/findimg/"+id_menu).toPromise();
  }
}
